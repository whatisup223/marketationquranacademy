    ...
    if (!empty($email) && !empty($password)) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            if ($user['role'] === 'admin') {
                ...
                header("Location: dashboard/index.php"); // Assuming a general dashboard exists
            }
            exit;
        } else {
            $error = t('البريد الإلكتروني أو كلمة المرور غير صحيحة', 'Invalid email or password');
        }
    } else {
        $error = t('يرجى ملء جميع الحقول', 'Please fill in all fields');
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>
        <?php echo t('تسجيل الدخول - أكاديمية أهل السنة', 'Login - Ahl El-Sona Academy'); ?>
    </title>
    <!-- Tailwind CSS -->
            ...
            <div class="p-10 rounded-[3rem] border backdrop-blur-2xl transition-all duration-500 relative shadow-2xl"
                ...
                <div class="text-center mb-10">
                    <h1 class="text-3xl font-black uppercase tracking-tighter mb-2"
                        :class="darkMode ? 'text-white' : 'text-slate-900'">
                        <?php echo t('تسجيل الدخول', 'Welcome Back'); ?>
                    </h1>
                    <p class="text-sm font-medium" :class="darkMode ? 'text-slate-400' : 'text-slate-500'">
                        <?php echo t('تابع رحلتك التعليمية في كتاب الله', 'Continue your journey in Quran sciences'); ?>
                    </p>
                </div>

                <?php if ($error): ?>
                    <div
                        class="mb-6 p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-500 text-sm font-bold text-center animate-shake">
                        <?php echo $error; ?>
                    </div>
                <?php endif; ?>
                ...
                <form method="POST" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center justify-between px-2">
                            ...
                            <a href="forgot-password.php"
                                class="text-[10px] font-black uppercase tracking-widest text-indigo-500 hover:text-indigo-400">
                                <?php echo t('نسيت كلمة السر؟', 'Forgot?'); ?>
                            </a>
                        </div>
                    ...
                    <div class="flex items-center gap-3 px-2">
                        <input type="checkbox" id="remember" name="remember"
                            class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 bg-transparent">
                        <label for="remember" class="text-xs font-bold cursor-pointer"
                            :class="darkMode ? 'text-slate-400' : 'text-slate-500'">
                            <?php echo t('تذكرني على هذا الجهاز', 'Remember me on this device'); ?>
                        </label>
                    </div>
                    ...
                    <button type="submit"
                        ...
                        <span><?php echo t('دخول الآن', 'Login Now'); ?></span>
